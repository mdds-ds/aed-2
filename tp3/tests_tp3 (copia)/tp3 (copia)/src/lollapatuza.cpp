...
lollapatuza::lollapatuza(map<IdPuesto ,puesto> Ps, set<Persona> Pers): puestos(Ps), personas(Pers), mayorgastador(*Pers.begin()), gastoPorPersona(
        gastoPorPersonaInicial(Pers)), puestosHackeables(), gastos(gastosIniciales(Pers)){};
...
map<Nat,map<Persona,Persona>> lollapatuza::gastosIniciales(set<Persona> per){
    map<Persona ,  Persona > a;
    for (auto it = per.begin(); it != per.end() ; it++) {
        a.insert({*it, *it});
    }
    map<Nat,map<Persona,Persona>> b;
    b.insert({0,a});
    return b;
}
...
void lollapatuza::registrarCompra(IdPuesto idPuesto, Persona per, Producto i, Nat &c) {
    ...
    if(gastoPorPersona.count(per)==1){
        Nat gastoAnt = gastoPorPersona.at(per);
        Nat gastoNuevo = gastoAnt + puestos.at(idPuesto).calcularCompra(i,c);
        gastoPorPersona.at(per)=gastoNuevo;
        gastos.at(gastoAnt).erase(per);
        if(gastos.at(gastoAnt).empty()){
            gastos.erase(gastoAnt);
    ...
    if(per!=mayorgastador && gastoNuevo>gastoPorPersona.at(mayorgastador)){
        ...
        if(per!=mayorgastador && gastoNuevo == gastoPorPersona.at(mayorgastador)){
            mayorgastador = min(per,mayorgastador);
        }
    }
